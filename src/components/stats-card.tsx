...
export const StatsCard = ({ title, timeframes }: Data) => {
  ...
  return (
    <div
      className={`${color} rounded-[15px] relative ${icon} bg-no-repeat bg-[position:right_10px_top_-10px] h-[160px] md:h-[213px] xl:h-[244px] `}
    >
      <div className='bg-Navy-900 rounded-[15px] p-6 absolute bottom-0 w-full'>
        <div className='flex items-center justify-between'>
          <p className='text-lg font-medium'>{title}</p>
          <button aria-label='More options' className='cursor-pointer'>
            <img
        ...
        <div className='flex justify-between items-center md:flex-col md:items-start'>
          <p className='text-[2rem] font-extralight md:text-[3.5rem]'>
            {timeframes[periodStore.period].current}hrs
          </p>
          <p className='text-Navy-200 text-[.9375rem]'>
